...
void binddown(node *head1 ,node *head0){
    node *now1 = head1 , *now0 = head0;
    head0 -> down = head1;
    for(int i=1;i<=m;i++){
        now1 = now1 -> right , now0 = now0 -> right;
        now0 -> down = now1; 
    }    
}
...
int main(){
    ...
    while(T--){
        ...
        change1(x1_1y1,x3_1y3,y2-y1+1); //修改下向,往右遍历
